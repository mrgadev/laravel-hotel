                ...
                <div class="flex w-full gap-5 mx-auto justify-between items-center">
                    <div class="flex flex-col gap-5 w-full p-6">
                        <div class="flex items-center gap-1 bg-primary-100 p-2 text-primary-700 rounded-lg w-fit text-sm">
                            ...
                            <p>Data Lokasi terdekat</p>
                        </div>
                        <h1 class="text-white text-4xl font-medium">
                            Data Lokasi terdekat
                        </h1>
                    </div>
                    <a href="{{route('dashboard.nearby-location.create')}}" class="flex items-center gap-2 mt-10 px-5 py-2 border-2 rounded-md bg-primary-100 p-2 text-primary-700 hover:bg-white transition-all duration-75 hover:text-[#976033] text-base text-center">
                ...
                <main class="col-span-12 md:pt-0">
                    <div class="p-10 mt-2 bg-white rounded-xl shadow-lg">
                        <table id="selection-table" class="">
                            ...
                            <tbody>
                                @forelse ($nearby_locations as $nearby_location)
                                    <tr class="cursor-pointer">
                                        <td scope="row" class="px-4 pe-0 py-4 font-medium text-gray-900 whitespace-nowrap">
                                            <input type="checkbox" name="location_ids[]" class="cursor-pointer child-checkbox w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600" value="{{ $nearby_location->id }}">
                                        </td>
                                        ...
                                        <td class="flex items-center">
                                            <div class="mr-2">
                                                <a href="{{route('dashboard.nearby-location.edit', $nearby_location)}}" class="py-2 px-2 border-2 rounded-md border-primary-600 text-primary-500 text-center transition-all hover:bg-primary-500 hover:text-white">
                                                    <i class="bi bi-pencil-square"></i>
                                                </a>
                                            </div>
                                            <form action="{{route('dashboard.nearby-location.destroy', $nearby_location)}}" class="" method="POST">
                                                @method('DELETE')
                                                @csrf
...
@push('addon-script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const actionUrl = "{{ route('dashboard.nearby-location.bulkDelete') }}";

            deleteButton.addEventListener('click', function () {
                const locationIds = Array.from(document.querySelectorAll('input[name="location_ids[]"]:checked'))
