                ...
                <div class="flex w-full gap-5 mx-auto justify-between items-center">
                    <div class="flex flex-col gap-5 w-full p-6">
                        ...
                        <h1 class="text-white text-4xl font-medium">
                            Data Rencana Penginapan
                        </h1>
                    </div>
                    <a href="{{route('dashboard.accomodation-plan.create')}}" class="mt-10 px-5 py-3 border-2 rounded-md bg-primary-100 p-2 text-primary-700 hover:bg-white transition-all duration-75 hover:text-[#976033] text-base text-center">
                ...
                <main class="col-span-12 md:pt-0">
                    <div class="p-10 mt-2 bg-white rounded-xl shadow-lg">
                        <table id="selection-table" class="">
                            ...
                            <tbody>
                                @forelse ($accomdation_plans as $accomdation_plan)
                                    <tr class="cursor-pointer">
                                        <td scope="row" class="px-4 pe-0 py-4 font-medium text-gray-900 whitespace-nowrap">
                                            <input type="checkbox" name="plan_ids[]" class="cursor-pointer child-checkbox w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600" value="{{ $accomdation_plan->id }}">
                                        </td>
                                        ...
                                        <td class="flex items-center">
                                            <div class="mr-2">
                                                <a href="{{route('dashboard.accomodation-plan.edit', $accomdation_plan)}}" class="py-2 px-2 border-2 rounded-md border-yellow-600 text-yellow-600 text-center">
                                                    <i class="bi bi-pencil-square"></i>
                                                </a>
                                            </div>
                                            <form action="{{route('dashboard.accomodation-plan.destroy', $accomdation_plan)}}" class="" method="POST">
                                                @method('DELETE')
                                                @csrf
                                                <button tyoe="submit" class="p-1 border-2 rounded-md border-red-600 text-red-600 text-center">
...
@push('addon-script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const actionUrl = "{{ route('dashboard.accomodation-plan.bulkDelete') }}";

            deleteButton.addEventListener('click', function () {
                const accomdationPlanIds = Array.from(document.querySelectorAll('input[name="plan_ids[]"]:checked'))
                    .map(checkbox => checkbox.value);

                if (accomdationPlanIds.length === 0) {
                    Swal.fire({
                        icon: "error",
                ...
                fetch(actionUrl, {
                    ...
                    body: JSON.stringify({ plan_ids: accomdationPlanIds })
                })
