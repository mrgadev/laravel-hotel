        ...
        <div class="container mx-auto">
            <div class="flex flex-col gap-5 w-full p-6">
                <div class="flex items-center gap-1 bg-primary-100 p-2 text-primary-700 rounded-lg w-fit text-sm">
                    ...
                    <a href="{{route('dashboard.room-facilities.index')}}" class="flex items-center hover:underline">
                        <p>Fasilitas Kamar</p>
                    </a>
                    <span class="material-symbols-rounded">chevron_right</span>
        ...
        <section class="container px-6 mx-auto">
            <main class="col-span-12 md:pt-0">
                <div class="p-2 mt-2 bg-white rounded-xl shadow-lg">
                    <form action="{{route('dashboard.room_facility.update', $room_facility)}}" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="">
                            <div class="px-4 py-5 sm:p-6">
                                <div class="mt-10">
                                    <div class="grid grid-cols-1 gap-10 sm:grid-cols-2">
                                        <div>
                                            <label for="name" class="block mb-3 font-medium text-gray-700 text-md">Nama Fasilitas Kamar</label>
                                            <input type="text" name="name" id="name" value="{{$room_facility->name}}" autocomplete="off" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                                    ...
                                    <div class="grid grid-cols-1 mt-5 items-center">
                                        <div>
                                            ...
                                            <textarea name="description" id="description" rows="10" cols="80">{!! $room_facility->description !!}</textarea>
        ...
        <div class="relative max-w-3xl mx-auto" onclick="event.stopPropagation()">
            <div>
                <img
                    ...
                    src="{{Storage::url($room_facility->icon)}}"
                />
            </div>
        </div>
    </a>
