        ...
        <main class="h-full overflow-y-auto">
            <div class="container mx-auto">
                <div class="flex w-full gap-5 mx-auto justify-between items-center">
                    ...
                    <a href="{{route('dashboard.room-facilities.create')}}" class="flex items-center gap-2 mt-10 px-5 py-2 border-2 rounded-md bg-primary-100 p-2 text-primary-700 hover:bg-white transition-all duration-75 hover:text-[#976033] text-base text-center">
                                ...
                                @forelse ($room_facilities as $room_facility)
                                    <tr class="cursor-pointer">
                                        <td scope="row" class="px-4 pe-0 py-4 font-medium text-gray-900 whitespace-nowrap">
                                            <input type="checkbox" name="facilities_ids[]" class="cursor-pointer child-checkbox w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600" value="{{ $room_facility->id }}">
                                        </td>
                                        ...
                                        <td class="flex items-center">
                                            <div class="mr-2">
                                                <a href="{{route('dashboard.room-facilities.edit', $room_facility)}}" class="py-2 px-2 border-2 rounded-md border-primary-600 text-primary-500 text-center transition-all hover:bg-primary-500 hover:text-white">
                                                    <i class="bi bi-pencil-square"></i>
                                                </a>
                                            </div>
                                            <form action="{{route('dashboard.room-facilities.destroy', $room_facility)}}" class="" method="POST">
                                                @method('DELETE')
                                                @csrf
...
@push('addon-script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const actionUrl = "{{ route('dashboard.room-facilities.bulkDelete') }}";
